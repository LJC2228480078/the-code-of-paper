...
A=[-0.021,-0.011,0.009,0.039;
   -0.031,-0.010,0.004,0.029;
   -0.0575,-0.0158,0.0048,0.0562;
   -0.0305,-0.0066,0.0045,0.0378;].*100;
 B(:,1)=A(:,2)-A(:,1);
 B(:,2)=A(:,3)-A(:,2);
 B(:,3)=A(:,4)-A(:,3);
 pp=4;
% 定义目标函数
fun_MV = @(x) fx_MV(x,B,A,pp);
fun_xinfangfa=@(x) fx_xinfangfa(x,B,A,pp);
fun_minkefusiji=@(x) fx_minkefusiji(x,B,A,pp);
fun_goldenrulefa=@(x) fx_goldenrulefa(x,B,A,pp);
fun_score=@(x) fx_score(x,B,A,pp);
fun_fangcha=@(x) fangcha(x,B,pp);
fun_qiwang=@(x) qiwang(x,A,pp);
fun_swekeness=@(x) swekeness(x,B,pp);
% 生成初始解
x0 = rand(1,pp);
...
H=zeros(10,4);
for i=1:10
fun_varaince=@(x)fx_varaince(x,B,pp,lamda(i));
...
L(i,1)=fun_qiwang(p);
L(i,2)=fun_fangcha(p);
L(i,3)=fun_swekeness(p);
L(i,4)=L(i,1)/sqrt(L(i,2));
end
...
%Expected Return
figure(1)
plot(H(:,2),H(:,1),"o-",L(:,2),L(:,1),"*-",K(:,2),K(:,1),"+-","LineWidth",1)
set(gca,"LineWidth",1.5)
legend("MV-Model","MagV-Model","Score V-Model",Location="northwest")
xlabel("Risk")
...
%Skewness
figure(3)
plot(K(:,2),K(:,4),"o-",H(:,2),H(:,4),"+-","LineWidth",1)
set(gca,"LineWidth",1.5)
legend("Score V-Model","MV-Model",Location="northwest")
xlabel("Risk")
ylabel("Sharpe Ratio")
